...
$_REQUEST["fonte_cor_fundo"] ? $background_color = '#'.$_REQUEST["fonte_cor_fundo"] : $background_color = '#990000';
$_REQUEST["fonte_cor"] ? $font_color = '#'.$_REQUEST["fonte_cor"] : $font_color = '#ffffff';
$_REQUEST["fonte_tamanho"] ? $font_size = $_REQUEST["fonte_tamanho"] : $font_size = 14;

$font_file  = 'fonte/CAFETA__.TTF';
...
if(!function_exists('ImageCreate'))
    fatal_error('Erro: Servidor não suporta a geração de imagem via PHP') ;

// clean up text
if(empty($_GET['text']))
    fatal_error('Erro: Sem texto especificado.') ;
...
if(!$font_found)
{
    fatal_error('Erro: Fonte especificada não encontrada.') ;
}
...
$box = @ImageTTFBBox(($font_size+2),3,$font_file,$text) ;
$image = @ImageCreate(abs($box[2]-$box[0]),abs($box[5]-$dip)) ;
if(!$image || !$box)
{
    fatal_error('Erro: O servidor não conseguiu criar a imagem.') ;
}
